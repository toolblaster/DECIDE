    ...
    init() {
        try {
            ...
            if (rD) {
                ...
                if (bD) {
                    console.warn('Restoring from silent backup.');
    ...
    updateUI(customMsg) {
        ...
        if (!entry || this.isEditing) {
            ...
            if (now.getHours() >= 18 && !entry.closure) {
                ...
                if (this.dom.statusText) this.dom.statusText.innerHTML = `<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg><span>${msg}</span>`;
